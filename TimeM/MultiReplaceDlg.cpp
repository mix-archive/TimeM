...
void CMultiReplaceDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_TARGETTEXT, m_strTarget);//这种方式必须调用updatedata
...
BEGIN_MESSAGE_MAP(CMultiReplaceDlg, CDialog)
	...
	ON_BN_CLICKED(IDOK, &CMultiReplaceDlg::OnBnClickedOk)
END_MESSAGE_MAP()
...
BOOL CMultiReplaceDlg::OnInitDialog()
{
	...
	UpdateCtrlStates();
	if(m_bForSelected)
			CheckRadioButton(IDC_RBTN_ALL,IDC_RBTN_SELECTED , IDC_RBTN_SELECTED);
		else
			CheckRadioButton(IDC_RBTN_ALL,IDC_RBTN_SELECTED , IDC_RBTN_ALL);
...
void CMultiReplaceDlg::OnBnClickedOk()
{

GetDlgItemText(IDC_EDIT_RESULTTEXT,m_strResult);
GetDlgItemText(IDC_EDIT_TARGETTEXT,m_strTarget);
m_bForSelected = IsDlgButtonChecked(IDC_RBTN_SELECTED);
